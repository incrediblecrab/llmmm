...
Generated by `tools/replicate.py`. Every number below is **recomputed from the shipped artefacts** (embeddings + `modes.json` + explorer labels). No value is taken from the paper's own result tables.
...
The model cards headline *"within-mode coherence 0.611 / 0.833 / 0.703 against a random-pair baseline of 0.097 / 0.348 / 0.115"* — margins of ~0.5.
...
Mean-cosine-to-centroid is arithmetically always larger than mean pairwise cosine over the same set, and restricting to a mode's 7 tightest members inflates it again. Measured consistently, the same modes score **0.414 / 0.656 / 0.454**, and mean pairwise over all members is only **0.162 / 0.424 / 0.198**.

Running the correct control — random ingredient groups **matched on mode size**, scored with the **same** statistic:
...
**The modes are real but weak.** They are more coherent than chance, consistently and in the same rank order the paper reports — but by ~0.06-0.09, not ~0.5.

### Why this matters for the product

Modes have a **tight core and a nearly-random tail** (median 89 members, max 254). The top ~7 members are genuinely coherent; membership past ~20 is close to noise.

> **Design rule: never render a mode's full membership. Show the top 8 by pole proximity and stop.** Anything else exposes the tail and the feature will read as broken. This also means "browse this mode" is a weak screen — modes are better as *labels* and *steering targets* than as *lists*.

---

## Second finding: aggregated poles collapse onto the corpus mean direction

Every supervised "direction" built by averaging mode poles ends up ~0.95 aligned with the global mean of the embedding matrix. Measured mean pairwise |cos|:
...
At 0.94–0.98 the ten sensory axes are effectively **one vector**. Raw top-K is the same list for every axis — corpus hubs:
...
Mean-centring drops collinearity to ~0.34 and makes results vivid — but the axis *labels* still don't hold (`bitter` returns orange and lime; `citrus` returns rosemary and thyme). **Verdict: do not ship labelled sensory sliders.**

Rule for the app: **single poles raw, aggregated poles centred**, and always eyeball a new aggregate before putting it in the UI.

---

## Third finding: the SLERP table is 81% reconstructible and still doesn't reproduce

The `epicure-explorer` Space source (`app.py::_aggregate_pole`) reveals how the author builds the missing `sweet` / `umami` / `high protein` directions. Applying that recipe lifts coverage of `direction_arithmetic_full.parquet` from **10% → 81.2%**. Agreement with the published table (`tools/slerp_via_space_recipe.py`):
...
θ=0 doesn't use the pole at all — it is pure `neighbors`, and it reproduces. Agreement then decays monotonically with rotation, which is the signature of a **wrong pole**: the shipped `supervised_poles.json` are not the vectors behind the published table.

Centring is *not* the missing step — it slightly helps θ>0 (0.345→0.368, 0.094→0.140) but badly hurts θ=0 (0.944→0.786), so the paper used raw cosine.

`floral`, `nutty` and `high water` match no pole family at all. The other 9 uncovered cells are exactly the §3 cuisine asymmetry — `Eastern_European` missing from cooc+chem (6 cells) and `Japanese` missing from core (3) — an independent confirmation.
