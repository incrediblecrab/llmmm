...
The publisher creates a public dataset and a **static** Space. It never requests paid hardware. It pins the dataset revision, builds a fresh app, runs the browser checks locally, uploads only the serving allowlist, then repeats the checks anonymously against the actual hosting origin returned by the Hub. This preserves the historical sample workflow; its guards refuse to replace the full ingredient dataset's current deployment.

Only after the live app works does it add demo links to the model card. All non-README model files and existing model tags must remain unchanged. A failed attempt leaves its intermediate upload receipts in the chosen artifact directory; it does not write a successful final receipt.

Use a new artifact directory and receipt for each release. `--update` permits an inspected update of the existing demo repositories; changed dataset bytes require a new `--dataset-tag`. Old tags are never moved. Regeneration and hosting stay local/Hugging Face; no GitHub Actions are used.
